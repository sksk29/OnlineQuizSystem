...
namespace DAL.Services
{
   public  class TeacherServices
    {
        ...
        private static TeacherServices _Instance;
 ...
 cat.category_name,  st.title AS status_title")
        ...
        public List<student_result> GetStudentsResultList(int? pageId, int pageSize, int? quiz_id)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    var ppSql = PetaPoco.Sql.Builder.Select(@"COUNT(*) OVER () as TotalRecordCount, quiz_q.quiz_id , std.student_id, std.first_name,   std.last_name, std.email,isnull(quiz.passing_marks,50) as passing_marks,
                        ...
                        .InnerJoin("quiz_questions AS quiz_q").On("sa.question_id = quiz_q.question_id ")
                         .InnerJoin("students AS std").On("sa.student_id = std.student_id ")
                          ...
                          .GroupBy("quiz_q.quiz_id, std.student_id, std.first_name,   std.last_name, std.email, quiz.passing_marks")
                        .OrderBy("std.student_id")
                      .Append(@"OFFSET (@0-1)*@1 ROWS
	                        FETCH NEXT @1 ROWS ONLY", pageId, pageSize);
        ...
        public student_result GetStudentsResultByQuizID_StudentID(int? quiz_id, int? student_id)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    var ppSql = PetaPoco.Sql.Builder.Select(@"top 1  quiz_q.quiz_id , std.student_id, std.first_name,   std.last_name, std.email,isnull(quiz.passing_marks,50) as passing_marks,
                       count(quiz_q.question_id) as total_questions,
                         (select count(quiz_q_correct.correct_option)*100/count(quiz_q.question_id) ) as std_percentage")
                        .From("student_answers AS sa")
                        .InnerJoin("quiz_questions AS quiz_q").On("sa.question_id = quiz_q.question_id ")
                         .InnerJoin("students AS std").On("sa.student_id = std.student_id ")
                          .LeftJoin("quiz_questions AS quiz_q_correct").On("sa.question_id = quiz_q_correct.question_id and sa.student_ans = quiz_q_correct.correct_option ")
                          .InnerJoin("quiz as quiz").On("quiz_q.quiz_id=quiz.quiz_id ")
                          .Where("quiz_q.quiz_id=@0 and std.student_id=@1", quiz_id, student_id)
                          .GroupBy("quiz_q.quiz_id, std.student_id, std.first_name,   std.last_name, std.email, quiz.passing_marks");
        ...
        public List<students> GetStudentsListsForTeachers(int? pageId, int pageSize, string user_name, string email, int? student_id)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    var ppSql = PetaPoco.Sql.Builder.Select(@"COUNT(*) OVER () as TotalRecordCount,  std.student_id, std.first_name, std.last_name,  std.user_name, std.email, std.is_active")
        ...
        public string UpdateAssignment(assignments FormData)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    ...
                    passing_marks =@6, modified_at =getdate(), modified_by =@7 where assignment_id=@8", FormData.title, FormData.start_date, FormData.end_date, FormData.allowed_time_minutes, FormData.status,
                        FormData.course_id,  FormData.passing_marks, FormData.modified_by, FormData.assignment_id);
        ...
        public List<student_assignment_result> GetStudentsAssignmentResultList(int? pageId, int pageSize, int? assignment_id)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    var ppSql = PetaPoco.Sql.Builder.Select(@" COUNT(*) OVER () as TotalRecordCount, ans.assign_answers_id, an.title as assignmentTitle,  isnull(ans.is_checked_by_teacher,0) as  is_checked_by_teacher  , an.total_marks, an.passing_marks,std.student_id, std.user_name, 
                        ...
                        .InnerJoin(" assignments an").On(" ans.assignment_id=an.assignment_id ")
                         .LeftJoin("students std").On(" ans.student_id=std.student_id ")
        ...
        public List<student_courses> GetStudentsListForCourse(int? pageId, int pageSize, int? course_id, string user_name, int? student_id)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    var ppSql = PetaPoco.Sql.Builder.Select(@" distinct COUNT(*) OVER () as TotalRecordCount,   std.student_id, std.user_name, std.email, cr.course_id, cr.title")
                        .From(" student_courses scr")
                        .InnerJoin("students std").On("scr.student_id=std.student_id")
                        .InnerJoin("courses cr").On("scr.course_id=cr.course_id")
            
                        .Where("scr.course_id=@0", course_id)
                        .Append(SearchParameters)
                        .OrderBy("std.student_id asc")
                       .Append(@"OFFSET (@0-1)*@1 ROWS
	                        FETCH NEXT @1 ROWS ONLY", pageId, pageSize);
