...
namespace DAL.Services
{
    ...
    public class AdminServices
    {
        ...
        public void LogErrorInDatabase(string error_message, string log_type)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
        ...
        public users UserLogin(string email, string password, string UserType)
        {
        ...
        public students StudentLogin(string email, string password)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    result = repo.Query<students>(ppSql).FirstOrDefault();
        ...
        public List<students> GetStudentsListsForAdmin(int? pageId, int pageSize, string user_name, string email, int? student_id)
        {
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                try
                {
                    ...
                    {
                        SearchParameters.Append("and std.student_id = @0 ", student_id);
                    }

                    #endregion

                    var ppSql = PetaPoco.Sql.Builder.Select(@"COUNT(*) OVER () as TotalRecordCount,  std.student_id, std.first_name, std.last_name,  std.user_name, std.email, std.is_active, std.password")
                        .From(" students AS std")

                        .Where("std.is_deleted=0")
                        .Append(SearchParameters)
                        .OrderBy("std.student_id desc")
                       .Append(@"OFFSET (@0-1)*@1 ROWS
	                        FETCH NEXT @1 ROWS ONLY", pageId, pageSize);


                    result = repo.Query<students>(ppSql).ToList();
        ...
        public string InsertNewStudent(students FormData)
        {
            string result = "";
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    if (FormData != null)
                    {
                        ...
                        var response = repo.Execute(@"INSERT INTO students(first_name, last_name, user_name, email, is_active, created_at, created_by, is_deleted, password)
                         VALUES(@0, @1, @2,@3, @4, getdate(), @5, 0 , @6)", FormData.first_name, FormData.last_name, FormData.user_name, FormData.email, FormData.is_active, FormData.created_by, FormData.password);
                        if (response > 0)
                        {
                            result = "Saved Successfully!";
                        }
        ...
        public string EditStudent(students FormData)
        {
            string result = "";
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    if (FormData != null)
                    {
                        ...
                        bool isNameExists = checkStudentUserNameBeforeUpdateStudent(FormData.user_name, FormData.student_id);
                        ...
                        bool isEmailExists = checkStudentEmailBeforeUpdateStudent(FormData.email, FormData.student_id);
                        if (isEmailExists == true)
                        {
                            result = "email already exists";
                            return result;
                        ...
                        var response = repo.Execute(@"update students
set first_name=@0, last_name=@1, user_name=@2, email=@3, is_active=@4, modified_at=getdate(), modified_by=@5 where student_id=@6",
FormData.first_name, FormData.last_name, FormData.user_name, FormData.email, FormData.is_active, FormData.created_by, FormData.student_id);
                        if (response > 0)
                        {
                            result = "Updated Successfully!";
                        }
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    if (!String.IsNullOrEmpty(user_name))
                    {
                        ...
                        var ppSql = PetaPoco.Sql.Builder.Append(@"select count(*) as ResponseMsg from students where  user_name =@0", user_name);
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    if (!String.IsNullOrEmpty(email))
                    {
                        ...
                        var ppSql = PetaPoco.Sql.Builder.Append(@"select count(*) as ResponseMsg from students where  email =@0", email);
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    if (!String.IsNullOrEmpty(user_name) || student_id < 1)
                    {
                        ...
                        var ppSql = PetaPoco.Sql.Builder.Append(@"select count(*) as ResponseMsg from students where  user_name =@0 AND student_id!=@1", user_name, student_id);
            ...
            using (var repo = DataContextHelper.GetPPContextHelper())
            {
                ...
                try
                {
                    if (!String.IsNullOrEmpty(email) || student_id < 1)
                    {
                        ...
                        var ppSql = PetaPoco.Sql.Builder.Append(@"select count(*) as ResponseMsg from students where  email =@0 AND student_id!=@1", email, student_id);
